// automation_tasks_rs for snake_bevy_wasm
...
{GREEN}cargo auto github_new_release{RESET} - {YELLOW}creates new release on GitHub{RESET}
  ...
  {YELLOW}© 2025 bestia.dev  MIT License github.com/bestia-dev/cargo-auto{RESET}
"#
    );
    print_examples_cmd();
}
...
r#"rsync -e ssh -a --info=progress2 --delete-after "web_server_folder/{package_name}/" "{luciano_bestia}@{bestia.dev}:/var/www/transfer_folder/{package_name}" "#).unwrap_or_else(|e| panic!("{e}"))
    .arg("{package_name}", &cargo_toml.package_name()).unwrap_or_else(|e| panic!("{e}"))
    .arg("{luciano_bestia}", "luciano_bestia").unwrap_or_else(|e| panic!("{e}"))
    .arg("{bestia.dev}", "bestia.dev").unwrap_or_else(|e| panic!("{e}"))    
    .run().unwrap_or_else(|e| panic!("{e}"));

    // rsync to copy to server over ssh the installation script
    cl::ShellCommandLimitedDoubleQuotesSanitizer::new(
r#"rsync -e ssh -a --info=progress2 --delete-after "publish_script/hello_world_publish.sh" "{luciano_bestia}@{bestia.dev}:/var/www/scripts/{package_name}/" "#).unwrap_or_else(|e| panic!("{e}"))
    .arg("{package_name}", &cargo_toml.package_name()).unwrap_or_else(|e| panic!("{e}"))
    .arg("{luciano_bestia}", "luciano_bestia").unwrap_or_else(|e| panic!("{e}"))
    .arg("{bestia.dev}", "bestia.dev").unwrap_or_else(|e| panic!("{e}"))    
    .run().unwrap_or_else(|e| panic!("{e}"));
    ...
    cl::ShellCommandLimitedDoubleQuotesSanitizer::new(
r#"ssh "{luciano_bestia}@{bestia.dev}" chmod +x  "/var/www/scripts/{package_name}/hello_world_publish.sh" "#).unwrap_or_else(|e| panic!("{e}"))
    .arg("{package_name}", &cargo_toml.package_name()).unwrap_or_else(|e| panic!("{e}"))
    .arg("{luciano_bestia}", "luciano_bestia").unwrap_or_else(|e| panic!("{e}"))
    .arg("{bestia.dev}", "bestia.dev").unwrap_or_else(|e| panic!("{e}"))    
    .run().unwrap_or_else(|e| panic!("{e}"));
    ...
    cl::ShellCommandLimitedDoubleQuotesSanitizer::new(
r#"ssh "{luciano_bestia}@{bestia.dev}" "/var/www/scripts/{package_name}/hello_world_publish.sh" "#).unwrap_or_else(|e| panic!("{e}"))
    .arg("{package_name}", &cargo_toml.package_name()).unwrap_or_else(|e| panic!("{e}"))
    .arg("{luciano_bestia}", "luciano_bestia").unwrap_or_else(|e| panic!("{e}"))
    .arg("{bestia.dev}", "bestia.dev").unwrap_or_else(|e| panic!("{e}"))    
    .run().unwrap_or_else(|e| panic!("{e}"));
