...
pub fn render_bird(queried_entities: Query<(&mut Bird, &mut Transform, &mut Sprite), Changed<Bird>>, game_board_canvas: Res<GameBoardCanvas>) {
    for (bird, mut transform, mut sprite) in queried_entities {
